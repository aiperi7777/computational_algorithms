...
function print(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    for (let j = 0; j < array.length + 1; j++) {
      if (j < array.length) {
        stroka +=
          array[i][j] + "*x" + koef + (array[i][j + 1] > 0 ? " + " : "   ");
        koef++;
      } else {
        stroka = stroka.slice(0, -2);
        stroka += "= " + array[i][j].toFixed(20); //.toFixed(20);
      }
    }
    mainStr += stroka + '<br/>'; //'<br/>';
  }
  console.log(mainStr);
...
function findSLAU(uravnenie, n, fl) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = i + 1; j < n; j++) {
      ...
      for (let k = n; k >= i; k--) {
        arrayForChange[j][k] = arrayForChange[j][k] - a * arrayForChange[i][k];
      }
    }
  }
  let array = [];
